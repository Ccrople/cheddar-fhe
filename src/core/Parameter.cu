...
Parameter<word>::Parameter(
  ...
  // Check if the primes are NTT-friendly. The conjugate-invariant ring needs a
  // 4N-th root of unity rather than a 2N-th one -- half of the primes an
  // ordinary parameter set may use are therefore unusable here, and a preset
  // carrying one of them would otherwise fail deep inside FindPrimitiveMthRoot.
  word ntt_base = static_cast<word>(CyclotomicIndex());
  auto assert_ntt_friendly = [ntt_base, this](word prime) {
    AssertTrue(prime % ntt_base == 1,
               std::string("Prime is not NTT-friendly") +
                   (conjugate_invariant_
                        ? " (the conjugate-invariant ring needs p = 1 mod 4N)"
                        : ""));
  };
  ...
  // 1. Populate galois factors. The Galois group acting on the slots is
  // (Z/M)^* / {+-1}, which is generated by 5 and has MaxNumSlots() elements:
  // N/2 for the ordinary ring at M = 2N, and N for the conjugate-invariant one
  // at M = 4N. Complex conjugation is the quotiented-out {+-1} and so acts
  // trivially on the real subring -- there is no HConj there to generate.
  int max_num_slots = MaxNumSlots();
  int m = CyclotomicIndex();
