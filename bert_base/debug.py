"""Compare the layer's tapped intermediates (BERT_BASE_DUMP) with the model.

    python debug.py <all_dir> <ref_dir with calib.json> <dump_dir> [layer]

Each `L<k>_<name>.f64` is [ct][live][T] in model units (the test divided by
the tap's factor and multiplied by the stream's public per-channel
suppression); this recomputes the same quantity in float64 from the recorded
prompt and prints rms relative error, worst |err| and a sample. The first
probe that is wrong is where the bug is.
"""
...
def host(L, name):
    ...
    # the GELU is one fit per feed-forward tile, in the permuted channel order
    g = c["gelu"]
    perm, tile = np.asarray(g["perm"]), g["tile"]
    a_g = np.empty(m.I)
    b_g = np.empty(m.I)
    for t, spec in enumerate(g["tiles"]):
        idx = perm[t * tile:(t + 1) * tile]
        a_g[idx] = 0.5 * (spec["hi"] - spec["lo"])
        b_g[idx] = 0.5 * (spec["hi"] + spec["lo"])
    ...
    # the feed-forward taps are the FIRST TILE only, in the permuted order
    if name == "t_gelu":
        idx = perm[:tile]
        return ((r["u"][0][:, idx] - b_g[idx]) / a_g[idx]).T
    if name == "g":
        return gelu(r["u"][0][:, perm[:tile]]).T
    if name == "y_ffn":
        return r["y"][0].T
    return None
