...
TEST(BootLanding, LandsLowerAndPreservesTheMessage) {
  ...
  // Full Boot additionally runs SlotToCoeff. The one thing that restricts it is
  // ci16_35's num_accum==1 zone: StC is a hoisted transform, and on the alpha-12
  // basis a hoisted transform in levels 0..6 produces mod-Q noise (Doing.md
  // 1.5bt), so Boot needs GetEndLevel() >= 7 -- odd L >= 11, even L >= 10. The
  // even landing's single-terminal (25-bit) CtS level no longer restricts it:
  // EvalSpecialFFT consumes that thin level with a pure rescale instead of a
  // transform phase (the even presets place it at the top, num_cts = 5), so both
  // parities land clean (~3e-5). HalfBoot -- the layer's crossing op, no StC --
  // stays valid at ANY L.
  const bool boot_ok = boot_land >= 7;
  if (boot_ok) {
    Ciphertext<word> res;
    EncryptAt(land, ct, msg, 0);
...
TEST(BootLanding, Ci16CiphertextCrossesInAndBackKeylessly) {
  ...
  // clears ci16_35's num_accum==1 zone (GetEndLevel() >= 7, either parity now),
  // else the layer's HalfBoot.
  const bool use_boot = b->GetBootParameter().GetEndLevel() >= 7;
  Ciphertext<word> res;
  if (use_boot)
    b->Boot(res, ct, land.ui->GetEvkMap());
  else
